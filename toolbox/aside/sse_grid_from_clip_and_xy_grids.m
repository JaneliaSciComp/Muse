function [sse_grid,a,vel,N_filt,V_filt,V]= ...
  sse_grid_from_clip_and_xy_grids(v,fs, ...
                                  f_lo,f_hi, ...
                                  Temp, ...
                                  x_grid,y_grid, ...
                                  R, ...
...
[sse,a]=sse_from_dfted_clip_and_delays_fast(V_filt,dt,tau,verbosity);

% % check just at the min
% [sse_min,i_argmin]=min(sse);
% tau_argmin=tau(:,i_argmin);
% [sse_min_check,a_check]= ...
%   sse_from_dfted_clip_and_delays(V_filt,dt,tau_argmin,verbosity);
% a
% a_check
% sse_min
% sse_min_check
