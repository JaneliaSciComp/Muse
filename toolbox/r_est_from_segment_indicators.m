...
% filter out the syl's that don't match i_segment
...
% package up all the trial overhead
overhead.R=R;
overhead.Temp=Temp;
overhead.dx=dx;
...
args_template_and_overhead=merge_scalar_structs(args_template,overhead);
...
for i_snippet=1:n_snippets ,
  ...
  snippet_args.syl_name=syl_name_keep{i_snippet};
  snippet_args.i_start=i_start_keep(i_snippet);
  snippet_args.i_end=i_end_keep(i_snippet);  
  snippet_args.f_lo=f_lo_keep(i_snippet);  
  snippet_args.f_hi=f_hi_keep(i_snippet);  
  snippet_args.r_head=r_head_keep(:,i_snippet);  
  snippet_args.r_tail=r_tail_keep(:,i_snippet);
  ...
  for i=1:length(args_field_names_to_add_to_blob)
    r_est_blob_this.(args_field_names_to_add_to_blob{i})=snippet_args.(args_field_names_to_add_to_blob{i});
  end
